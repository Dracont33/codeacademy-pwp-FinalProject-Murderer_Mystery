...
def get_average_sentence_length(text):
    ...
    sentence_lengths = 0
    for x in sentences_in_text:
        sentence_lengths += len(x.split(" "))
...
def build_frequency_table(corpus):
    ...
    for x in corpus:
        if x not in frequency_table:
            frequency_table[x] = corpus.count(x)
    return frequency_table

def ngram_creator(text_list):
    x=0
    word_pairs = []
    while (x+1) < len(text_list):
        word_pairs.append(" ".join([text_list[x], text_list[(x+1)]]))
        x += 1
    return word_pairs
...
class TextSample:
    ...
    def __repr__(self):
        return "The sample was written by {name} and it has an average of {num} words per sentence".format(name = self.author, num = self.average_sentence_length)
...
#print(murderer_sample.prepare_text)
...
def suspect_scores(sample, murderer_sample):
    score = find_text_similarity(sample, murderer_sample)
    return "{name} got a {score} score.".format(name = sample.author, score = score)
    
print(suspect_scores(lily_sample, murderer_sample))
print(suspect_scores(myrtle_sample, murderer_sample))
print(suspect_scores(gregg_sample, murderer_sample))
...
def who(samples,murderer_sample):
    highest_score=[0,0]
    for x in samples:
        score = find_text_similarity(x, murderer_sample)
        if score > highest_score[0]:
            highest_score[0] = score
            highest_score[1] = x.author
    return "The murderer is {name} who got a score of {score}.".format(name = highest_score[1], score = highest_score[0])

suspects_notes = [lily_sample, myrtle_sample, gregg_sample]
print(who(suspects_notes,murderer_sample))
